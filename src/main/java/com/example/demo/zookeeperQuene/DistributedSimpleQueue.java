...
import com.example.demo.zookeeper.BaseDistributedLock;
 ...
 * @author linbingwen
 *先进先出队列
 * 在zookeeper中先创建一个根目录 queue_fifo，做为队列。入队操作就是在queue_fifo下创建自增序的子节点，
 * 并把数据放入节点内。出队操作就是先找到queue_fifo下序号最下的那个节点，取出数据，然后删除此节点。
 * @param <T>
 */
